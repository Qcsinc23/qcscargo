...
Deno.serve(async (req) => {
    ...
    try {
        ...
        console.log(`Admin ${authResult.user!.email} updating quote ${quote_id} to status: ${status}`);

        // Update quote status using service role (bypasses RLS)
        const updateResponse = await fetch(
            `${supabaseUrl}/rest/v1/shipping_quotes?id=eq.${quote_id}`,
            {
                method: 'PATCH',
                headers: {
        ...
        console.log(`Successfully updated quote ${quote_id} to status: ${status}`);
        ...
        return createErrorResponse('QUOTE_UPDATE_FAILED', error instanceof Error ? error.message : 'An error occurred', 500);
    }
});
