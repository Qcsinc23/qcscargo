...
export default function HowItWorks() {
  const steps = [
    ...
    },
    {
      ...
      icon: <Truck className="h-12 w-12 text-slate-600" />,
      duration: "Same day"
    },
    {
      number: 3,
    ...
    },
    {
      ...
      icon: <Plane className="h-12 w-12 text-slate-600" />,
              ...
              {steps.map((step, index) => (
                <div key={index} className="relative">
                  ...
                  {index < steps.length - 1 && (
                    <div className="absolute left-8 top-20 w-0.5 h-16 bg-slate-200 hidden md:block"></div>
                  )}
                  
                  <div className="bg-white rounded-xl shadow-lg p-8 border-l-4 border-slate-600">
                    <div className="flex flex-col md:flex-row items-start space-y-4 md:space-y-0 md:space-x-6">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="bg-slate-600 text-white w-8 h-8 rounded-full flex items-center justify-center font-bold">
                          {step.number}
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                          <h3 className="text-2xl font-bold text-slate-800 mb-2 md:mb-0">{step.title}</h3>
                          <span className="text-sm bg-green-100 text-green-800 px-3 py-1 rounded-full font-medium">
                            {step.duration}
                          </span>
                        </div>
                        
                        <p className="text-slate-600 text-lg mb-4">{step.description}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {documentationRequired.map((doc, index) => (
                <div key={index} className="bg-white p-6 rounded-lg shadow-lg">
                  <h3 className="text-lg font-semibold text-slate-800 mb-2">{doc.title}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {packagingGuidelines.map((guideline, index) => (
                <div key={index} className="bg-white p-6 rounded-lg shadow-lg border-l-4 border-green-500">
                  <h3 className="text-lg font-semibold text-slate-800 mb-2">{guideline.title}</h3>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="bg-white p-6 rounded-lg shadow-lg">
                <h3 className="text-xl font-semibold text-slate-600 mb-2">Processing</h3>
                <p className="text-3xl font-bold text-slate-800 mb-2">1-2 Days</p>
                <p className="text-slate-600">Quote to ready for shipping</p>
              </div>
              
              <div className="bg-white p-6 rounded-lg shadow-lg">
                <h3 className="text-xl font-semibold text-slate-600 mb-2">Transit</h3>
                <p className="text-3xl font-bold text-slate-800 mb-2">3-7 Days</p>
                <p className="text-slate-600">Air freight to Caribbean</p>
              </div>
              
              <div className="bg-white p-6 rounded-lg shadow-lg">
                <h3 className="text-xl font-semibold text-slate-600 mb-2">Delivery</h3>
                <p className="text-3xl font-bold text-slate-800 mb-2">1-3 Days</p>
                <p className="text-slate-600">Customs clearance to delivery</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link 
              ...
              className="bg-yellow-500 text-slate-900 px-8 py-4 rounded-lg text-lg font-bold hover:bg-yellow-400 transition-colors inline-flex items-center justify-center"
            >
              Calculate Shipping Cost <ArrowRight className="ml-2 h-5 w-5" />
            </Link>
            <Link 
              to="/contact" 
              className="border-2 border-white text-white px-8 py-4 rounded-lg text-lg font-semibold hover:bg-white hover:text-slate-900 transition-colors inline-flex items-center justify-center"
            >
              Contact Our Team
            </Link>
          </div>
